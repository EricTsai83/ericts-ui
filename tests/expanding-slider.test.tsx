...
function renderSlider(
  props: Partial<ExpandingSliderProps> = {},
  children?: React.ReactNode,
) {
  const utils = render(
    <ExpandingSlider label="Volume" defaultValue={50} {...props}>
      <ExpandingSliderTrigger aria-label="Mute">
        <span>icon</span>
      </ExpandingSliderTrigger>
      {children ?? <ExpandingSliderTrack />}
    </ExpandingSlider>,
  );
  const slider = screen.getByRole("slider", { name: "Volume" });
  const root = slider.closest("[data-slot='expanding-slider']") as HTMLElement;
  const query = (slot: string) =>
    root.querySelector(`[data-slot='expanding-slider-${slot}']`) as HTMLElement;
...
describe("ExpandingSlider", () => {
  ...
  it("re-arms a pending collapse against a changed delay", () => {
    ...
    rerender(
      <ExpandingSlider label="Volume" defaultValue={50} collapseDelay={500}>
        <ExpandingSliderTrigger aria-label="Mute">
          <span>icon</span>
        </ExpandingSliderTrigger>
        <ExpandingSliderTrack />
  ...
  it("collapses at once when the delay drops below the elapsed time", () => {
    ...
    rerender(
      <ExpandingSlider label="Volume" defaultValue={50} collapseDelay={100}>
        <ExpandingSliderTrigger aria-label="Mute">
          <span>icon</span>
        </ExpandingSliderTrigger>
        <ExpandingSliderTrack />
  ...
  it("routes trigger clicks to the consumer", () => {
    ...
    render(
      <ExpandingSlider label="Volume">
        <ExpandingSliderTrigger aria-label="Mute" onClick={onClick}>
          <span>icon</span>
        </ExpandingSliderTrigger>
        <ExpandingSliderTrack />
      </ExpandingSlider>,
  ...
  it("anchors the track to the trailing edge when it precedes the trigger", () => {
    render(
      <ExpandingSlider label="Volume" expanded>
        <ExpandingSliderTrack align="end" />
        <ExpandingSliderTrigger aria-label="Mute">
          <span>icon</span>
        </ExpandingSliderTrigger>
      </ExpandingSlider>,
  ...
  describe("composition", () => {
    it("publishes rail geometry as CSS variables for replacement parts", () => {
      ...
      expect(rail.style.getPropertyValue("--expanding-slider-thumb-size")).toBe(
        `${THUMB_SIZE}px`,
      );
      expect(
        rail.style.getPropertyValue("--expanding-slider-thumb-offset"),
      ).toBe(`${travel / 2}px`);
      expect(rail.style.getPropertyValue("--expanding-slider-fill")).toBe(
    ...
    it("lets the rail, range, and thumb be replaced", () => {
      const { slider, rail, thumb } = renderSlider(
        {},
        <ExpandingSliderTrack>
          <ExpandingSliderRail className="bg-red-500">
            <ExpandingSliderRange className="bg-blue-500" />
            <ExpandingSliderThumb className="rounded-none" />
          </ExpandingSliderRail>
        </ExpandingSliderTrack>,
      );
    ...
    it("fails loudly when a part is rendered outside the root", () => {
      ...
      expect(() => render(<ExpandingSliderTrack />)).toThrow(
        /must be rendered inside <ExpandingSlider>/,
      );
