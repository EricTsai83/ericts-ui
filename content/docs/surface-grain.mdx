...
without changing their color, layout, or information hierarchy. This page walks
through the flatness it addresses, why the texture helps, how the two compare
side by side, and the complete CSS to add it to a shadcn project.
...
## The Problem With Perfectly Flat Surfaces

A digital surface filled with one exact color has no variation anywhere in it.
Every pixel of a `#0a0a0a` sidebar is identical to every other pixel. Nothing
in the physical world looks like that, so large expanses of it tend to read as
empty color fields rather than as material the interface sits on.

That flatness shows up most in the places users stare at longest:

- **App shells and sidebars.** Persistent chrome covers a large share of the
  viewport and rarely changes, so an unbroken color field is on screen the
  entire session.
- **Near-white light themes.** Wide areas of near-white can feel clinical, and
  the usual fix—adding a tint or an accent—changes the palette instead of the
  surface.
- **Near-black dark themes.** Deep backgrounds lose the sense of a lit surface
  entirely and can look like a hole behind the content.
- **Editors and dashboards.** Restrained, mostly neutral tools have little
  imagery to break up the background, which leaves the flatness exposed.

The usual responses make the interface louder: another border, another
elevation step, a gradient, a background image. Each one adds a visual element
that competes with the content it surrounds.

## Why Grain Helps

Grain is a fine, repeated texture placed over the flat background. It
introduces small variations across the area, giving the eye enough detail to
read it as a surface—without changing color, layout, or hierarchy.

The treatment works because it stays below the interface hierarchy:
...
Each pair below holds the content, layout, and surface color constant. The left
...
The pattern geometry stays the same in both themes: a `256 × 256` tile,
`baseFrequency=".9"`, four octaves, and stitched edges. Only the opacity
changes, because the same noise reads differently against a pale surface than
against a dark one:
...
These values are visual starting points, not universal constants. A single
shared value such as `0.035` also works if you prefer one definition; otherwise,
tune each theme against your actual `background`, `card`, and `sidebar` colors.
