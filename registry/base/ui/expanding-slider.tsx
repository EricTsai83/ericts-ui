...
type ExpandingSliderContextValue = {
  value: number;
  min: number;
  max: number;
  step: number;
...
const ExpandingSliderContext =
  React.createContext<ExpandingSliderContextValue | null>(null);

function useExpandingSliderContext(component: string) {
  const context = React.useContext(ExpandingSliderContext);

  if (!context) {
    throw new Error(`${component} must be rendered inside <ExpandingSlider>.`);
  }

  return context;
}

export type ExpandingSliderProps = Omit<
 ...
 * revealed content from `ExpandingSliderTrigger` and `ExpandingSliderTrack`;
...
export function ExpandingSlider({
  ...
  const context = React.useMemo<ExpandingSliderContextValue>(
    () => ({
      value: currentValue,
      min,
      max,
  ...
  return (
    <ExpandingSliderContext.Provider value={context}>
      <div
        data-slot="expanding-slider"
        ...
        className={cn(
          ...
          "group/expanding-slider inline-flex w-fit items-center rounded-full border border-transparent bg-clip-padding",
    ...
    </ExpandingSliderContext.Provider>
  );
}

export type ExpandingSliderTriggerProps = React.ComponentProps<"button">;

/** The always-visible control. Pass any icon as `children`. */
export function ExpandingSliderTrigger({
  className,
  style,
  disabled,
  type = "button",
  ...props
}: ExpandingSliderTriggerProps) {
  const context = useExpandingSliderContext("ExpandingSliderTrigger");
  ...
  return (
    <button
      type={type}
      disabled={isDisabled}
      data-slot="expanding-slider-trigger"
      className={cn(
        "inline-flex shrink-0 items-center justify-center rounded-full text-foreground outline-none transition-colors",
        "[&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-(--expanding-slider-icon-size)",
      ...
      style={
        {
          width: TRIGGER_SIZE,
          height: TRIGGER_SIZE,
          "--expanding-slider-icon-size": `${ICON_SIZE}px`,
          ...style,
...
export type ExpandingSliderTrackProps = React.ComponentProps<"div"> & {
 ...
 */
export function ExpandingSliderTrack({
  align = "start",
  className,
  style,
  children,
  ...
  onKeyDown,
  ...props
}: ExpandingSliderTrackProps) {
  const context = useExpandingSliderContext("ExpandingSliderTrack");
  const {
    commitValue,
    disabled,
    dragging,
  ...
  return (
    <div
      data-slot="expanding-slider-panel"
      className={cn(
    ...
    >
      <div
        ...
        data-slot="expanding-slider-track"
        className={cn(
      ...
      >
        {children ?? (
          <ExpandingSliderRail>
            <ExpandingSliderRange />
            <ExpandingSliderThumb />
          </ExpandingSliderRail>
        )}
      </div>
    </div>
...
export type ExpandingSliderRailProps = React.ComponentProps<"div">;
 ...
 */
export function ExpandingSliderRail({
  className,
  style,
  ...props
}: ExpandingSliderRailProps) {
  const { railRef, ratio, trackWidth } = useExpandingSliderContext(
    "ExpandingSliderRail",
  );
  ...
  return (
    <div
      ref={railRef}
      aria-hidden="true"
      data-slot="expanding-slider-rail"
      className={cn("relative w-full rounded-full bg-primary/20", className)}
      style={
        {
          height: RAIL_HEIGHT,
          "--expanding-slider-thumb-size": `${THUMB_SIZE}px`,
          "--expanding-slider-thumb-offset": `${thumbOffset}px`,
          // The fill stops at the thumb's centre, not its leading edge.
          "--expanding-slider-fill": `${thumbOffset + THUMB_SIZE / 2}px`,
          ...style,
        } as React.CSSProperties
      }
      {...props}
...
export type ExpandingSliderRangeProps = React.ComponentProps<"div">;

/** The filled portion of the rail. */
export function ExpandingSliderRange({
  className,
  ...props
}: ExpandingSliderRangeProps) {
  return (
    <div
...
export type ExpandingSliderThumbProps = React.ComponentProps<"div">;

/** The handle. It collapses with the surface so it is never clipped mid-reveal. */
export function ExpandingSliderThumb({
  className,
  ...props
}: ExpandingSliderThumbProps) {
  return (
    <div
