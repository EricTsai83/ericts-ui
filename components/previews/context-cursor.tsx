...
export default function Preview() {
  ...
  return (
    <div className="flex w-full max-w-xl flex-col gap-3">
      {/* ContextCursor stays inert unless the device reports a fine pointer, so
          on a touch screen the demo below looks like three plain cards. Say so
          rather than letting it read as a broken preview. */}
      <p className="hidden rounded-md border border-dashed bg-muted/40 px-3 py-2 text-xs leading-5 text-muted-foreground pointer-coarse:block">
        Needs a mouse. The cursor badge follows a fine pointer, so it stays
        hidden on touch screens.
      </p>

      <ContextCursor className="w-full rounded-lg border bg-background p-3">
